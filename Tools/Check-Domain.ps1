...
try { 
    ...
    try { # This test sometimes returns true if it hasn't tried and failed yet once.
        $DomainResult = $null
        $DomainResult = Test-ComputerSecureChannel -ErrorAction SilentlyContinue # This also can return true if the domain was never joined
...
finally { # This runs even though exit codes are already passed.
